...
RenderBufferObject* rtt_fbo;
//0.2. The Projection and view matrices that will control the location of our camera
glm::mat4 P,V;
//0.3. RTT Projection and view matrices
glm::mat4 rttP,rttV;
//0.4. The OpenGL contents that we are going to display
OpenGLContent *objectA;
OpenGLContent *objectB;
OpenGLContent *rttMonitor;
...
int main( void )
{
	...
	do{
		renderWorldScene(rtt_fbo,rttP,rttV); //A. Render the scene from the render to texture camera
		renderWorldScene(def_fbo,P,V); //B. Render the scene from the normal camera
		OpenGLFramework::postRender(); //C. Ask OpenGL to display the final render (as soon as it is ready). We use "Double buffer".
		updateWorldScene();			   //D. React to user's inputs, update your animations...
	} 
	while( OpenGLFramework::programEnded());// E. Check if the ESC key was pressed or the window was closed
...
void createWorldScene(){
	...
	rtt_fbo=RenderBufferObject::createRTTBufferObject(800,600);
	...
	//3 Finally we set the projection and view matrices for the RTT
	rttV = glm::lookAt(glm::vec3(0,10,0), glm::vec3(0,0,0), glm::vec3(0,0,-1));
	rttP = glm::perspective(45.0f, 4.0f / 3.0f, 0.1f, 100.0f);
}
